"""
TESPy-based CHP Steenwijk thermodynamic and scenario engine.

This module keeps the same public API as the earlier CoolProp-based engine:
- analyse_cycle(...)
- compute_scenario(...)
- run_all_scenarios(...)
- SCENARIO_DEFS

The thermodynamic core is now solved with TESPy. The model represents a
single-extraction CHP steam turbine train:
- live steam source
- HP turbine
- one extraction split
- LP condensing branch to the main condenser
- optional LP screw expander on the extraction branch

Useful process heat is reported with the same proxy as the earlier app:
extraction-side enthalpy above the feedwater reference state. For expander
cases, the post-expander state is used.
"""
...
from tespy.components import Condenser, Sink, Source, Splitter, Turbine
...
def _state_from_conn(name: str, conn: Connection) -> SteamState:
    ...
    return SteamState(
        ...
        x=x,
        phase=_phase_from_quality(float(x_val)),
    )
...
def _solve_open_chp_network(
    ...
    """Solve an open TESPy network of the CHP turbine train."""
    ...
    c5 = Connection(lp, "out1", cond, "in1", label="5")
    c6 = Connection(cond, "out1", cond_sink, "in1", label="6")
    ...
    if has_lp_expander:
        ...
        c3e = Connection(exp, "out1", ex_sink, "in1", label="3e")
        nw.add_conns(c3, c3e)
        ...
        exp = None
        c3 = Connection(split, "out1", ex_sink, "in1", label="3")
        nw.add_conns(c3)
        c3.set_attr(m=m_ext)

    hp.set_attr(eta_s=eta_HP)
    lp.set_attr(eta_s=eta_LP)
    cond.set_attr(pr1=1.0, pr2=0.99)
    ...
    nw.solve("design")
    if nw.status != 0:
        raise RuntimeError("TESPy could not solve the CHP network for the selected scenario.")

    result = {
        "state1": _state_from_conn("1_LiveSteam", c1),
        "state2": _state_from_conn("2_HP_Actual", c2),
        "state3": _state_from_conn("3_Extraction_or_Process", c3 if not has_lp_expander else c3e),
        "state4": _state_from_conn("4_LP_Inlet", c4),
        "state5": _state_from_conn("5_LP_Actual", c5),
        "state6": _state_from_conn("6_Condensate", c6),
        "P_hp_shaft_kW": abs(float(hp.P.val)) / 1000.0,
        "P_lp_shaft_kW": abs(float(lp.P.val)) / 1000.0,
        "P_exp_shaft_kW": abs(float(exp.P.val)) / 1000.0 if exp is not None else 0.0,
        "cooling_water_kg_s": float(cw1.m.val),
    }
    if has_lp_expander:
        result["state3e"] = _state_from_conn("3e_Expander", c3e)
    return result
...
def compute_scenario(
    ...
    solved_network = _solve_open_chp_network(
        steam_tph=steam_tph,
    ...
    P_HP = solved_network["P_hp_shaft_kW"] * eta_gen
    P_LP = solved_network["P_lp_shaft_kW"] * eta_gen
    P_exp = solved_network["P_exp_shaft_kW"] * eta_gen if has_lp_expander else 0.0
...
if __name__ == "__main__":
    ...
    print("=" * 88)
    print("CHP STEENWIJK — TESPY-BASED PLANT PERFORMANCE MODEL")
    print("=" * 88)
    for r in results:
        print(
